...
import { Button, Card, Modal, Form, Input, Select, TimePicker, message } from 'antd';
...
const Restaurant = () => {
  ...
  return (
    <div>
      ...
      <CreateRestaurantModal isModalOpen={isModalOpen} setIsModalOpen={setIsModalOpen} />
    </div>
  );
};
...
function CreateRestaurantModal({ isModalOpen, setIsModalOpen }: IProps) {
  ...
  const onFinish = async (values: any) => {
    try {
      ...
      if (data) {
        console.log('Uploaded image to storage and created a restaurant record on DB');
        console.log(data);
      }
  ...
  return (
    ...
    >
      ...
      <Form name="basic" layout="vertical" onFinish={onFinish} onFinishFailed={onFinishFailed} autoComplete="off">
        <Form.Item label="Name" name="name" rules={[{ required: true, message: 'Please input restaurant name!' }]} style={{ marginBottom: 10 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          <Form.Item
            ...
            name="restaurantType"
            rules={[{ required: true, message: 'Please input restaurant type!' }]}
